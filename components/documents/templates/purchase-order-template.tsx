...
export function PurchaseOrderTemplate({
  ...
  const vendorName = data.customer_name || document.customer_name || ""
  const vendorEmail = data.customer_email || ""
  const vendorAddress = data.customer_address || ""
  const vendorPhone = data.customer_phone || ""
  const vendorTaxId = data.customer_tax_id || ""
    ...
    <div className="font-sans text-black p-10 leading-relaxed">
      ...
      <div className="flex justify-between items-start mb-12">
        <div className="space-y-3">
          <h1 className="text-4xl font-bold uppercase tracking-tight text-gray-800 mb-1">PURCHASE ORDER</h1>
      ...
      {/* Vendor and Purchase Order Details Section */}
      <div className="grid grid-cols-2 gap-12 mb-12">
        <div className="space-y-2 bg-gray-50 p-6 rounded-lg shadow-sm">
          <h2 className="text-lg font-bold text-gray-700 mb-3 uppercase tracking-wide">Vendor:</h2>
          <p className="font-medium text-lg">{vendorName}</p>
          {vendorEmail && <p className="text-gray-600">{vendorEmail}</p>}
          {vendorAddress && (
            <p className="whitespace-pre-line text-gray-600 mt-2">{vendorAddress}</p>
          )}
          {vendorPhone && <p className="text-gray-600">{vendorPhone}</p>}
          {vendorTaxId && (
            <p className="font-medium mt-2">TIN: {vendorTaxId}</p>
          )}
        </div>
        
        <div className="bg-gray-50 p-6 rounded-lg shadow-sm">
          <h2 className="text-lg font-bold text-gray-700 mb-3 uppercase tracking-wide text-right">Order Details:</h2>
          <div className="grid grid-cols-2 gap-2">
            <p className="font-semibold text-gray-700 text-right">PO Date:</p>
            ...
            <p className="text-right">{document.due_date ? format(new Date(document.due_date), "dd/MM/yyyy") : "To be confirmed"}</p>
            
            {data.reference && (
              <>
                <p className="font-semibold text-gray-700 text-right">Reference:</p>
                <p className="text-right">{data.reference}</p>
              </>
            )}
            
            {data.delivery_address && (
              <>
                <p className="font-semibold text-gray-700 text-right">Delivery Address:</p>
                <p className="text-right whitespace-pre-line">{data.delivery_address}</p>
              </>
            )}
